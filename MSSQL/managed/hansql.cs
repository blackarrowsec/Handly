...
{
    public static string run(string a, string dname)
    {
        string result;
		int n;
        try 
	...
	protected static string RunThings (string hnd, string dname)
	{
...
{
    public static string run(string dname)
    {
        var res = "";
		var fullName = "";
		...
		{
	        if (assem.FullName.StartsWith(dname))
		    {	
    	...
    	try
	   	{
		    ...
		    res += "[!] Assembly loaded, now executing:\n";
		    Type type = assem.GetType("program");
               res += type.GetMethod("run").Invoke(null, null);
	    }
    	catch (Exception e)
	   	{
	    	res += e.Message + "\n";
